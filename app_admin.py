...
def outlet_import():
    if request.method == 'POST':
        if 'csv_file' not in request.files:
            flash('No file selected', 'danger')
            current_app.logger.warning('Outlet import attempted without file selection')
            return redirect(request.url)

        file = request.files['csv_file']
        if file.filename == '':
            flash('No file selected', 'danger')
            current_app.logger.warning('Outlet import attempted with empty filename')
            return redirect(request.url)
        ...
        import_session_id = str(uuid.uuid4())[:8]
        error_details = []
        current_app.logger.info(f'Starting outlet import session {import_session_id} for file: {file.filename}')
        
        try:
            # Read CSV file using built-in csv module
            if not file.filename.endswith('.csv'):
                flash('Please upload a CSV file only. Excel files are not supported in this version.', 'danger')
                current_app.logger.warning(f'Import {import_session_id}: Non-CSV file attempted: {file.filename}')
                return redirect(request.url)
            ...
            if not fieldnames:
                flash('CSV file appears to be empty or has no header row', 'danger')
                current_app.logger.error(f'Import {import_session_id}: CSV file has no header row')
                return redirect(request.url)
            
            current_app.logger.info(f'Import {import_session_id}: CSV file read successfully')
            
            # Validate required columns
            required_columns = ['urn', 'outlet_name', 'region']
            missing_columns = [col for col in required_columns if col not in fieldnames]
            if missing_columns:
                error_msg = f'File missing required columns: {", ".join(missing_columns)}'
                flash(error_msg, 'danger')
                current_app.logger.error(f'Import {import_session_id}: {error_msg}. Available columns: {", ".join(fieldnames)}')
                return redirect(request.url)

            # Log column validation success
            current_app.logger.info(f'Import {import_session_id}: Column validation passed. Columns found: {", ".join(fieldnames)}')
            
            # Convert CSV data to list for processing
            csv_data = list(csv_reader)
            row_count = len(csv_data)
            current_app.logger.info(f'Import {import_session_id}: {row_count} data rows found')
            ...
            # Validate required regions (optional - adjust based on your business rules)
            valid_regions = ['SW', 'SE', 'NC', 'NW', 'NE', 'SS']  # Add South-South if needed
            
            # Helper function to safely get and clean values
            def safe_get_value(row_dict, key, default=''):
            ...
            for index, row in enumerate(csv_data, start=2):  # Start at 2 to account for header row
                row_num = index
                try:
                    # Debug log the raw row data (only log first few rows to avoid spam)
                    if index <= 4:
                        current_app.logger.info(f'Import {import_session_id}: Row {row_num} sample data: {dict(row)}')
                    
                    # Validate required fields - handle None values
                    urn = safe_get_value(row, 'urn')
                    outlet_name = safe_get_value(row, 'outlet_name')
                    region = safe_get_value(row, 'region').upper()
                    
                    # Check for empty required fields
                    if not urn or not outlet_name or not region:
                        error_msg = f'Row {row_num}: Missing required fields - URN: "{urn}", Outlet Name: "{outlet_name}", Region: "{region}"'
                        error_details.append(error_msg)
                        current_app.logger.warning(f'Import {import_session_id}: {error_msg}')
                        error_count += 1
                        continue
                    
                    # Validate region if you have specific allowed regions
                    if region not in valid_regions:
                        error_msg = f'Row {row_num}: Invalid region "{region}" for URN {urn}. Valid regions: {", ".join(valid_regions)}'
                        error_details.append(error_msg)
                        current_app.logger.warning(f'Import {import_session_id}: {error_msg}')
                        error_count += 1
                        continue
                    
                    # Validate URN format (basic validation - adjust pattern as needed)
                    if not urn.startswith('DCP/'):
                        error_msg = f'Row {row_num}: Invalid URN format "{urn}". URN should start with "DCP/"'
                        error_details.append(error_msg)
                        current_app.logger.warning(f'Import {import_session_id}: {error_msg}')
                        error_count += 1
                        continue
                    ...
                    if existing:
                        # Get current values from database to preserve existing data for empty fields
                        c.execute('''
                        SELECT outlet_name, customer_name, address, phone, outlet_type, local_govt, state, region
                        FROM outlets WHERE urn = ?
                        ''', (urn,))
                        current_data = c.fetchone()

                        # Use new data if provided, otherwise keep current data
                        outlet_name = outlet_name if outlet_name else current_data[0]
                        customer_name = safe_get_value(row, 'customer_name') or current_data[1]
                        address = safe_get_value(row, 'address') or current_data[2]
                        phone = safe_get_value(row, 'phone') or current_data[3]
                        outlet_type = safe_get_value(row, 'outlet_type') or current_data[4]
                        local_govt = safe_get_value(row, 'local_govt') or current_data[5]
                        state = safe_get_value(row, 'state') or current_data[6]
                        region = region if region else current_data[7]
                        ...
                        WHERE urn = ?
                        ''', (
                            outlet_name, customer_name, address, phone,
                            outlet_type, local_govt, state, region, urn
                        ))
                        update_count += 1
                        current_app.logger.info(f'Import {import_session_id}: Row {row_num} - Updated existing outlet URN: {urn}')
                    else:
                        # Prepare data for insert with safe handling
                        insert_data = (
                            urn,
                            outlet_name,
                            safe_get_value(row, 'customer_name'),
                            safe_get_value(row, 'address'),
                            safe_get_value(row, 'phone'),
                            safe_get_value(row, 'outlet_type'),
                            safe_get_value(row, 'local_govt'),
                            safe_get_value(row, 'state'),
                            region
                        )
                        
                        # Log the data being inserted (for debugging)
                        current_app.logger.info(f'Import {import_session_id}: Row {row_num} - Inserting data: URN={insert_data[0]}, Name={insert_data[1]}, Customer={insert_data[2]}, Address={insert_data[3][:50]}...')
                        ...
                        ''', insert_data)
                        success_count += 1
                        current_app.logger.info(f'Import {import_session_id}: Row {row_num} - Successfully inserted outlet URN: {urn}, Name: {outlet_name}')

                except sqlite3.IntegrityError as e:
                    error_msg = f'Row {row_num}: Database integrity error for URN {urn}: {str(e)}'
                    error_details.append(error_msg)
                    current_app.logger.error(f'Import {import_session_id}: {error_msg}')
                    error_count += 1
                except Exception as e:
                    error_msg = f'Row {row_num}: Unexpected error processing URN {urn}: {str(e)}'
                    error_details.append(error_msg)
                    current_app.logger.error(f'Import {import_session_id}: {error_msg}')
                    error_count += 1
            ...
            current_app.logger.info(
                f'Import {import_session_id} completed: '
                f'{total_processed} rows processed, '
                f'{success_count} new outlets created, '
                f'{update_count} existing outlets updated, '
            ...
            if error_count > 0:
                flash(
                    f'Import completed with {error_count} errors. '
                    f'Successfully created {success_count} new outlets and updated {update_count} existing outlets. '
                    f'Check application logs for detailed error information.',
                    'warning'
                )
                # Log first few errors in flash message for immediate user feedback
                if error_details:
                    error_summary = '\n'.join(error_details[:5])  # Show first 5 errors
                    if len(error_details) > 5:
                        error_summary += f'\n... and {len(error_details) - 5} more errors (see logs)'
                    current_app.logger.error(f'Import {import_session_id} error summary:\n{error_summary}')
            else:
                flash(f'Import completed successfully! Created {success_count} new outlets and updated {update_count} existing outlets.', 'success')
            ...
            error_msg = f'File encoding error: {str(e)}. Please ensure the CSV file is UTF-8 encoded.'
            flash(error_msg, 'danger')
            current_app.logger.error(f'Import {import_session_id}: {error_msg}')
            return redirect(request.url)
        except Exception as e:
