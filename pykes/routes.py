...
from flask import render_template, request, redirect, url_for, flash, jsonify, session, send_from_directory, make_response, send_file, current_app
...
def init_routes(app):
    ...
    def outlets():
        ...
        # Debug logging for field agent access
        current_app.logger.info(f"Outlets access - User: {session.get('username')}, Role: {session.get('role')}, Region: {session.get('region')}, State: {session.get('state')}")
        ...
        if session['role'] == 'field_agent':
            # Case-insensitive region comparison only - don't filter by state
            query += " AND UPPER(o.region) = UPPER(?)"
            count_query += " AND UPPER(o.region) = UPPER(?)"
            params.append(session['region'])

        # Add user-selected filters (case-insensitive)
        if region:
            query += " AND UPPER(o.region) = UPPER(?)"
            count_query += " AND UPPER(o.region) = UPPER(?)"
        ...
        if state:
            ...
            count_query += " AND UPPER(o.state) = UPPER(?)"
            params.append(state)

        if local_govt:
            query += " AND UPPER(o.local_govt) = UPPER(?)"
            count_query += " AND UPPER(o.local_govt) = UPPER(?)"
            params.append(local_govt)

        if outlet_type:
            query += " AND UPPER(o.outlet_type) = UPPER(?)"
            count_query += " AND UPPER(o.outlet_type) = UPPER(?)"
        ...
        # Debug: Log results
        current_app.logger.info(f"Query returned {len(outlets)} outlets out of {total_outlets} total available")
        if len(outlets) > 0:
            current_app.logger.info(f"First outlet: URN={outlets[0]['urn']}, Name={outlets[0]['outlet_name']}, Region={outlets[0]['region']}")
        else:
            if session['role'] == 'field_agent' and total_outlets == 0:
                current_app.logger.warning(f"No outlets available for field agent {session['username']} in region {session['region']}")
                flash(f"No outlets are currently available in your assigned region ({session['region']}). Please contact your administrator.", 'info')
            else:
                current_app.logger.warning("No outlets returned - this may indicate a filtering issue")
